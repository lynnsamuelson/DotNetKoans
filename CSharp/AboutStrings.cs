...
broken line";
        ...
        public void ACrossPlatformWayToHandleLineEndings()
        {
            ...
            var vebatimString = "I\r\nam a\r\nbroken line";  // LS NOTE: should be @"I ...with actual returns
		...
		public void StringsCanBePaddedToTheLeft()
		{
			...
			var str = string.Format("{0,3:}", "x");  //output is 3 total characters so 2 spaces and 1 for the "x"
			Assert.Equal("  x", str);
		}
		...
		{
			var str = string.Format("{0,-3:}", "x"); //Negative signs pad to the right.  So put the string and then add spaces.
			Assert.Equal("x  ", str);
		}
		...
		{
			var str = string.Format("{0:n3}", 123456); //add separators n stands for number This requests the first item and to treat like a #
			Assert.Equal("123,456.000", str); //so insert comas every 3 numbers.  2 decimals is automatic, but can add a number after the n to say the number of decimals
		}

		[Koan(17)]
		public void CurrencyDesignatorsCanBeAdded()
		{
			var str = string.Format("{0:c}", 123456);//without a number after the n requires 2 decimals
			Assert.Equal("$123,456.00", str); //c means currancy and needs a $ in front of the string  and can also use g to indicate a straight number with no commas
		}
		...
		{
			var str = string.Format("{0:t}", DateTime.Parse("12/16/2011 2:35:02 PM")); //"t" will do hour, min and PM or AM
        ...
        {
            var str = @"\n";//everything you type is considered a character
